...
public class RTCCompleteTeamBuild extends Recorder {
	...
	if (build.getDownstreamBuilds().size() > 0 && build.getResult() == Result.SUCCESS)
	    return true;
	...
	try {
	    resolvedBuildResultUUID = TokenMacro.expandAll(build, listener, getBuildResultUUID());
	...
	try {
	    writeCompleteTeamBuildFile(resolvedBuildResultUUID, envs, antScriptFilePath);
	} catch (Exception e) {
	    // TODO Auto-generated catch block
	    e.printStackTrace();
	}
    ...
    private void writeCompleteTeamBuildFile(String resolvedBuildResultUUID, EnvVars envs,
