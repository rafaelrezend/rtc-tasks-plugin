...
import jenkins.plugins.teamant.rtc.tasks.StartTeamBuildTask;
...
public class RTCStartBuild extends Builder {
	...
	String resultUUID = parseContent(StringUtils.join(build.getLog(5).toArray()), regex);
	...
	build.replaceAction(tbAction.merge(new RTCTeamBuildAction(resultUUIDProperty, resultUUID)));
	
	
	// Add variable to the Jenkins environment.
//        List<ParameterValue> envVars = new ArrayList<ParameterValue>();
//        envVars.add(new StringParameterValue(resultUUIDProperty, resultUUID));
//        build.addAction(new ParametersAction(envVars));
    ...
    private void writeStartTeamBuildFile(EnvVars envs,
	...
	antDoc.addEcho(this.resultUUIDProperty + "=${" + this.resultUUIDProperty + "};");
